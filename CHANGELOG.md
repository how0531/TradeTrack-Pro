...
### Fixed — UX Phase 1 快速止血（13 項）+ 重複匯入判定修正

UX 全面審查（13 審查代理 + 行號級事實查核，36 項確認）的第一批修復，
聚焦「使用者信任」與常用路徑上的高影響低風險項。

#### 交易表單（TradeModal）
- 金額欄 `step="0.1"` 會讓兩位小數（1234.56）觸發原生驗證泡泡、儲存
  完全卡死 → 改 `step="any"`；vibrate('success') 從按鈕 onClick 移到
  驗證通過後，不再在儲存失敗時給「成功」震動。
- 編輯券商同步交易時損益(pts/%)欄一律空白（"+150 pts" 被 number input
  的 value sanitization 清成空字串，看起來像資料遺失）→ parseFloat 顯示。

#### 通知與版面
- Toast portal 到 document.body + z-[10010] — 同步期間發出的所有通知
  （含「部分帳號同步失敗」警告）不再被 z-9999 的 modal 整片遮住。
- 右上角眼睛按鈕、Toast 堆疊、底部導覽列補 safe-area-inset — 不再壓進
  iPhone 狀態列/動態島/home indicator。

#### 設定頁（SettingsView）
- 「雲端還原」二段確認加 2 秒冷卻鎖定（比照重置面板），快速連點無法
  擊穿；確認文案寫明「以雲端資料覆蓋本機」；「雲端備份」排到前面，
  兩鈕各加方向說明（本機→雲端 / 雲端→本機）。
- 「已備份至 Google Cloud」徽章改依 syncStatus 切換（同步中/失敗/尚未
  同步），不再永遠綠勾假保證；備份時間戳含日期（今天/昨天/M/D HH:mm）。
- 備份/還原結果從原生 alert 改為 toast。

#### 隱私
- 分享卡「隱藏金額」匯出的圖片其實沒有隱藏（html2canvas 不支援 CSS
  blur，只剩 50% 透明度）→ 改文字遮罩 '****'，預覽與匯出一致。
- 策略分析列表無視「隱藏金額」明碼顯示損益 → 接上 hideAmounts。

#### 券商同步（SyncDateModal）
- 回饋時機修正：預覽階段的 toast 從「已匯入」改為「已取得 N 筆，請
  檢核後匯入」；真正確認匯入完成後才發成功 toast（帶實際筆數與略過數）。
- 90 天上限/未來日期/起訖顛倒改為送出前本地驗證，不再等完冷啟動才被打回。
- 勾選 0 筆時不再能「確認匯入」空集合。
- 錯字「已實作損益」→「已實現損益」。

#### 重複匯入判定修正（使用者回報「辨識怪怪的」）
- **同步匯入的方法A 只比 orderNo**：Shioaji 的 orderNo 實為查詢結果列
  序號（0,1,2...）非全域唯一 — 既有交易的「3」會跟新批次的第 4 筆誤
  命中，新交易被誤標「已存在」自動取消勾選。補上「同單號還須同日+
  同代號」防護（v3.9.1 H7 修了「移除重複」，同步匯入這條路漏修）。
- **單號不同 ≠ 非重複**：列序號跨批次會位移，同一筆交易兩次同步可能
  拿到不同序號 — 方法A 不命中時改為落到方法B 內容比對，而非直接判定
  非重複（真重複漏抓、重複匯入）。
- **重複匯入不再洗掉使用者標註**：確認匯入是整筆覆蓋寫入，同 ID 交易
  重新匯入會把先前補的策略/心態清空、手寫筆記被機器 note 覆蓋、已刪除
  的交易復活。改為：軟刪除的交易跳過不匯入（刪除優先）；strategy/
  emotion 保留既有值；非機器格式的 note（使用者改寫過）保留。
