            ...
            <div className="space-y-4">
                 ...
                 <div className="p-5 rounded-2xl bg-black/20 border border-white/5 space-y-4">
                     ...
                     <div className="flex items-start gap-3">
                         <div className={`mt-1 w-2 h-2 rounded-full ${syncStatus === 'synced' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : syncStatus === 'error' ? 'bg-red-500 animate-pulse' : 'bg-yellow-500 animate-bounce'}`} />
                         <div className="flex-1">
                             <div className="text-xs font-bold text-white mb-1">
                                 {syncStatus === 'synced' ? '雲端連線正常' : syncStatus === 'error' ? '連線異常 / 權限受限' : '正在通訊...'}
                             </div>
                             {syncError && (
                                 <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-[10px] text-red-400 font-mono leading-relaxed mt-2">
                                     ERROR: {syncError}
                                 </div>
                             )}
                         </div>
                     ...
                     <div className="space-y-3">
                         <p className="text-[10px] text-slate-500 leading-relaxed">
                            如果您遇到「Unexpected state」或「權限不足」報錯，這可能是瀏覽器資料庫鎖定或 Firebase 規則過期。請嘗試下方的修復工具。
                         </p>
                         <button 
                            onClick={() => {
                                if (window.confirm('此操作將會清除瀏覽器本地快取並重新載入。您的交易資料在雲端是安全的，只需重新登入即可。要繼續嗎？')) {
                                    repairDatabase();
                                }
                            }}
                            className="w-full py-3 rounded-xl bg-amber-500/10 border border-amber-500/20 hover:bg-amber-500 hover:text-black hover:border-amber-500 text-amber-500 text-[10px] font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                         >
