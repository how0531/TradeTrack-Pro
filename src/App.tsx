                ...
                <div className="bg-[#D05A5A]/10 border-b border-[#D05A5A]/30 backdrop-blur-md px-4 py-2 flex items-center justify-between sticky top-0 z-50 animate-in slide-in-from-top duration-500">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-[#D05A5A] uppercase tracking-wide">
                             {isDDAlert 
                                ? (lang === 'zh' ? `?žæ’¤ ${formatDecimal(Math.abs(metrics.currentDD))}% å·²é?è­¦æ? (${ddThreshold}%)` : `Drawdown ${formatDecimal(Math.abs(metrics.currentDD))}% hits limit (${ddThreshold}%)`)
                                : (lang === 'zh' ? `??? ${riskStreaks.currentLoss} æ¬¡ï?å»ºè­°?«å?äº¤æ?` : `Lost ${riskStreaks.currentLoss} in a row. Take a break.`)
                             }
                                ? (lang === 'zh' ? `?žæ’¤ ${formatDecimal(Math.abs(metrics.currentDD))}% å·²é?è­¦æ? (${ddThreshold}%)` : `Drawdown ${formatDecimal(Math.abs(metrics.currentDD))}% hits limit (${ddThreshold}%)`)
                                : (lang === 'zh' ? `??? ${riskStreaks.currentLoss} æ¬¡ï?å»ºè­°?«å?äº¤æ?` : `Lost ${riskStreaks.currentLoss} in a row. Take a break.`)
                                }
                    ...
                    <button onClick={() => isDDAlert ? setDdThreshold(99) : setMaxLossStreak(99)} className="text-[10px] text-[#D05A5A] underline opacity-80 hover:opacity-100">{lang === 'zh' ? 'å¿½ç•¥' : 'Dismiss'}</button>
                </div>
            )}
